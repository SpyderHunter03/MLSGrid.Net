...
The unofficial [MLSGrid][mlsgrid] .NET library, supporting .NET Standard 2.0+, .NET Core 2.0+, and .NET Framework 4.6.1+.
...
```sh
dotnet add package MLSGrid.Net
```

Using the [NuGet Command Line Interface (CLI)][nuget-cli]:

```sh
nuget install MLSGrid.Net
```

Using the [Package Manager Console][package-manager-console]:

```powershell
Install-Package MLSGrid.Net
```
...
4. Click on the *Browse* tab and search for "MLSGrid.Net".
5. Click on the MLSGrid.Net package, select the appropriate version in the
...
```c#
GridConfiguration.GridClient = new GridClient(
    apiKey,
