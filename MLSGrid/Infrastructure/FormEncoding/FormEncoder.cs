...
namespace MLSGrid
{
    ...
    internal static class FormEncoder
    {
        ...
        {
            List<KeyValuePair<string, object>> flatParams = value switch
            {
                null => SingleParam(keyPrefix, string.Empty),
                IAnyOf anyOf => FlattenParamsAnyOf(anyOf, keyPrefix),
                INestedOptions options => FlattenParamsOptions(options, keyPrefix),
                IDictionary dictionary => FlattenParamsDictionary(dictionary, keyPrefix),
                string s => SingleParam(keyPrefix, s),
                Stream s => SingleParam(keyPrefix, s),
                IEnumerable enumerable => FlattenParamsList(enumerable, keyPrefix),
                DateTime dateTime => SingleParam(keyPrefix,((DateTimeOffset)dateTime).ToUnixTimeSeconds().ToString(CultureInfo.InvariantCulture)),
                Enum e => SingleParam(keyPrefix, JsonUtils.SerializeObject(e).Trim('"')),
                _ => SingleParam(keyPrefix, string.Format(CultureInfo.InvariantCulture, "{0}", value)),
            };
            return flatParams;
        }
        ...
        private static List<KeyValuePair<string, object>> FlattenParamsAnyOf(
            ...
            string keyPrefix)
        {
            List<KeyValuePair<string, object>> flatParams = new();
