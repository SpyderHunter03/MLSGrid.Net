...
namespace MLSGrid
{
    ...
    /// </summary>
    public class GridStreamedResponse : GridResponseBase
    {
        private Task<GridResponse> fetchFullyTask;

        public GridStreamedResponse(HttpStatusCode statusCode, HttpResponseHeaders headers, Stream body)
        ...
        public async Task<GridResponse> ToStripeResponseAsync()
        {
        ...
        private async Task<GridResponse> FetchResponseAsTextAsync()
        {
            var reader = new StreamReader(this.Body);
            var content = await reader.ReadToEndAsync().ConfigureAwait(false);
            return new GridResponse(this.StatusCode, this.Headers, content);
        }
    }
}
