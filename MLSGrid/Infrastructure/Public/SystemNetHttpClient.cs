...
namespace MLSGrid
{
    ...
    public class SystemNetHttpClient : IHttpClient
    {
        ...
        private static readonly Lazy<HttpClient> LazyDefaultHttpClient
            = new(BuildDefaultSystemNetHttpClient);
        ...
        public async Task<GridResponse> MakeRequestAsync(
            GridRequest request,
        ...
        public async Task<GridStreamedResponse> MakeStreamingRequestAsync(
            GridRequest request,
            CancellationToken cancellationToken = default)
        {
            var (response, retries) = await this.SendHttpRequest(request, cancellationToken).ConfigureAwait(false);

            return new GridStreamedResponse(
                response.StatusCode,
                response.Headers,
                await response.Content.ReadAsStreamAsync().ConfigureAwait(false))
            {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                this.requestTelemetry.MaybeAddTelemetryHeader(request.GridHeaders);
            }
        ...
        private string BuildStripeClientUserAgentString()
        {
            ...
            {
                { "bindings_version", GridConfiguration.StripeNetVersion },
                { "lang", ".net" },
                { "publisher", "stripe" },
        ...
        {
            var userAgent = $"Stripe/v1 .NetBindings/{GridConfiguration.StripeNetVersion}";
        ...
        private HttpRequestMessage BuildRequestMessage(GridRequest request)
        {
            ...
            foreach (var header in request.GridHeaders)
            {
