...
namespace MLSGrid
{
    ...
    public class GridClient : IGridClient
    {
        ...
        public async Task<T> RequestAsync<T>(
            ...
            where T : IGridEntity
        {
            var request = new GridRequest(this, method, path, options, requestOptions);
        ...
        private static T ProcessResponse<T>(GridResponse response)
            where T : IGridEntity
        {
            if (response.StatusCode != HttpStatusCode.OK)
            {
            ...
            try
            {
                obj = GridEntity.FromJson<T>(response.Content);
            }
            ...
            obj.GridResponse = response;

            return obj;
        }

        private static GridException BuildStripeException(GridResponse response)
        {
            ...
            var stripeError = errorToken.Type == JTokenType.String
                ? GridError.FromJson(response.Content)
                : GridError.FromJson(errorToken.ToString());

            stripeError.GridResponse = response;

            return new GridException(
                response.StatusCode,
                stripeError,
        ...
        private static GridException BuildInvalidResponseException(GridResponse response)
        {
            return new GridException(
                response.StatusCode,
                null,
